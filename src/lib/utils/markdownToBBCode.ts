			...
			if (lowerValue.startsWith('<details')) {
				...
				if (lowerValue.includes('</details>')) {
					...
					if (summaryMatch && summaryMatch[1]) {
						result += `[SPOILER=${summaryMatch[1].trim()}]`;
					} else {
						...
						if (i + 1 < children.length) {
							...
							if (
								...
								if (nextSummaryMatch && nextSummaryMatch[1]) {
									result += `[SPOILER=${nextSummaryMatch[1].trim()}]`;
...
function renderNode(
	...
	switch (node.type) {
		...
		case 'strong': {
			...
			const isUnderline =
				node.position &&
				raw.slice(node.position.start.offset ?? 0, (node.position.start.offset ?? 0) + 2) === '__';
