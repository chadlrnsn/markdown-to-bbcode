...
import { markdownToBBCode } from '../src/lib/utils/markdownToBBCode';
import { bbcodeToHtml } from '../src/lib/utils/bbcodeToHtml';
import { editorStore } from '../src/lib/stores/editor.svelte';
...
describe('editorStore modes', () => {
	it('should disable lists, tables, spoilers, and hr in chat mode and keep them in result mode', () => {
		editorStore.markdownText = `
- Item 1
...
`;
		...
		// Test Result mode
		editorStore.mode = 'result';
		editorStore.syncBBCode();
		let bbcode = editorStore.bbcodeText;
		expect(bbcode).toContain('[LIST]');
		expect(bbcode).toContain('[TABLE]');
		expect(bbcode).toContain('[SPOILER=Spoiler]');
		expect(bbcode).toContain('[HR]');

		// Test Chat mode
		editorStore.mode = 'chat';
		editorStore.syncBBCode();
		bbcode = editorStore.bbcodeText;
		// In Chat mode: lists, tables, spoilers, and hr should be bypassed/disabled
		expect(bbcode).not.toContain('[LIST]');
		expect(bbcode).not.toContain('[TABLE]');
